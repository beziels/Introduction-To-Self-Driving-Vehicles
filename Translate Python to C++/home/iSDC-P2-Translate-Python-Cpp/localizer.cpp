...
vector < vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  ...
  float cell_belief = 1.0 / (grid_height * grid_width);
	newGrid = zeros(grid_height, grid_width);

  for(int i = 0; i < grid_height; i++){
    for(int j = 0; j < grid_width; j++){
      newGrid[i][j] = cell_belief;
    }
  }
	return newGrid;
}
  ...
  float blurring) 
{
  ...
  vector < vector <float> > newGrid;
  int grid_height = beliefs.size();
  int grid_width = beliefs[0].size();
  ...
  for(int i = 0; i < beliefs.size(); i++){
    for(int j = 0; j < beliefs.size(); j++){
        ...
        int i_update = (i + dy + grid_height) % grid_height;
        int j_update = (j + dx + grid_width) % grid_width;
        
        newGrid[i_update][j_update] = beliefs[i][j];
    }
  }
