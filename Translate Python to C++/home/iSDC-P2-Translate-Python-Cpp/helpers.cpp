...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// summing of all values in the grid
	...
	for(int i = 0; i < m; i++){
		for(int j = 0; j < n; j++){
			newGrid[i][j] = grid[i][j] / grid_total;
		}
	}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	// ... 18 possible updates: 2/3 corner, 1/3 adjacent
	float corner_probabilities = blurring/12.0;
	float adjacent_probabilities = blurring/6.0;
	float center_probabilities = 1.0-blurring; 

	vector <vector <float> > grid_probabilities(3, vector <float> (3,0));
	
	// row 1 & 3 are very similar in the grid
	grid_probabilities[0][0] = corner_probabilities;
	grid_probabilities[0][1] = adjacent_probabilities;
	grid_probabilities[0][2] = corner_probabilities;

	// row 2, with the center
	grid_probabilities[1][0] = adjacent_probabilities;
	grid_probabilities[1][1] = center_probabilities;
	grid_probabilities[1][2] = adjacent_probabilities;

	// row 3 in the grid
	grid_probabilities[2][0] = corner_probabilities;
	grid_probabilities[2][1] = adjacent_probabilities;
	grid_probabilities[2][2] = corner_probabilities;
	...
	for(int i=0; i < grid_height; i++){
		...
		for(int j=0; j < grid_width; j++){
			float cell_value = grid[i][j];
			
			for(int row_change = -1; row_change  < 2; row_change++){
				
				for(int col_change = -1; col_change < 2; col_change ++){
					float cell_update = grid_probabilities[row_change+1][col_change+1];
					
					int i_update = (i + col_change + grid_height) % grid_height;
					int j_update = (j + row_change + grid_width) % grid_width;
					
					newGrid[i_update][j_update] += cell_update * cell_value;
				}
			}
		}
	}
